...
PROCESS_THREAD(cc2538_pwm_test, ev, data)
{
  ...
  char R_buf[14],G_buf[14],B_buf[14];
...
//               pwm_num[i].ab);
 ...
 //              pwm_num[i].ab);
      ...
      sprintf(R_buf,"R: %3u",pwm_num[0].duty);
      sprintf(G_buf,"G: %3u",pwm_num[1].duty);
      sprintf(B_buf,"B: %3u",pwm_num[2].duty);
      LCD_write_english_string(0,0,"RGB setting");
      LCD_write_english_string(0,1,R_buf);
      LCD_write_english_string(0,2,G_buf);
      LCD_write_english_string(0,3,B_buf);
